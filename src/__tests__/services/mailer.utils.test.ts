...
describe("utils", () => {
  it("webhook check", async () => {
    ...
    const isLegit = MailerUtils.verifyWebHook(body, signature, secret);
    expect(isLegit).toBeTruthy();
  });
  it("no body", async () => {
    try {
      MailerUtils.verifyWebHook(undefined as any, "test", "test");
    } catch (e: any) {
  ...
  it("no signature", async () => {
    try {
      MailerUtils.verifyWebHook(Buffer.from("test"), undefined as any, "test");
    } catch (e: any) {
  ...
  it("no secret", async () => {
    try {
      MailerUtils.verifyWebHook(Buffer.from("test"), "test", undefined as any);
    } catch (e: any) {
